...
def serialize_lua(doc: Document) -> str:
    ...
    Returns a string ending in a newline; an empty document returns just
    the header.
        ...
        if isinstance(line, Source):
            for sub_doc in line.documents:
                ...
                if sub_lua_path is not None:
                    state.extras.append(f"dofile({quote_string(str(sub_lua_path))})")
            continue
...
def _assemble_lua(state: _EmitState) -> str:
    ...
    Each non-empty bucket on *state* becomes one section; sections are
    joined with a blank line between them. The header sits at the top,
    followed by the section list — no sentinel comparisons needed.
    """
    sections: list[str] = []

    if state.config_tree:
        sections.append(f"hl.config({format_table(state.config_tree, indent=0)})\n")
    if state.extras:
        sections.append("".join(f"{call}\n" for call in state.extras))
    if state.exec_start:
        sections.append(format_exec_block("hyprland.start", state.exec_start))
    if state.exec_shutdown:
        sections.append(format_exec_block("hyprland.shutdown", state.exec_shutdown))
    if state.skipped:
    ...
    if isinstance(line, SectionClose):
        if state.section_stack:
            ...
            if close_name == "device" and close_buf is not None:
                state.extras.append(f"hl.device({format_table(close_buf, indent=0)})")
            elif close_name in ("windowrule", "windowrulev2") and close_buf is not None:
                state.extras.append(f"hl.window_rule({format_table(close_buf, indent=0)})")
            elif close_name == "layerrule" and close_buf is not None:
                state.extras.append(f"hl.layer_rule({format_table(close_buf, indent=0)})")
        return
    ...
    if isinstance(line, Assignment):
        if state.section_stack:
            ...
            if cur_name in _BLOCK_RULE_SECTIONS and cur_buf is not None:
                ...
                return
        set_nested(state.config_tree, split_key(line.full_key), coerce_value(line.value))
        return
...
def _process_keyword(line: Keyword, state: _EmitState, owning_doc: Document) -> None:
    ...
    if name in ("exec", "exec-once", "exec-shutdown"):
        ...
        bucket = state.exec_shutdown if name == "exec-shutdown" else state.exec_start
